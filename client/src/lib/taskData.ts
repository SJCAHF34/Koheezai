...
export const TASKS: PharmacyTask[] = [
  // ── DATA ENTRY TECH · DAILY ─────────────────────────────────────────────
  {
    ...
    description: "Clear 'Reply Received' — notify pts of refill denials, mark on CPS priority comments\nWork through 'Phone' contact method — resend requests or call prescriber for unresolved items\nRespond to ClerkChat messages — resolve open requests, escalate clinical questions to RPh",
  ...
  },
  {
    id: "de-d-003",
    title: "ICQ Queue",
    description: "Sort through ICQ — process new Rxs, refill responses, PSAs, and transferred prescriptions",
    role: "data_entry_tech",
    frequency: "daily",
    category: "operations",
    taskGroup: "ICQ",
  ...
  },
  {
    id: "de-d-008",
    title: "Support",
    description: "Assist other staff with DE, PV2, DP, CM, ICQ tasks as needed\nComplete SupplyLogix cycle count to track on-hand inventory",
    role: "data_entry_tech",
    frequency: "daily",
    category: "operations",
  ...
  {
    id: "pv2-d-001",
    title: "CRC & Order Review",
    description: "Resolve CRC email kickbacks and reprocess rejected orders\nFilter orders by delivery method (will call, FedEx/USPS, driver) — omit CONTACT PT/PA submitted; review for completeness and merge split orders\nCheck CPS priority comments — confirm packaging/delivery method, note any temp address changes\nPush DE/Adj Exc items and message RPh for DUR/PV1 clinical review",
    role: "pv2_tech",
    frequency: "daily",
    category: "operations",
  ...
  {
    id: "pv2-d-005",
    title: "Dispensing & Copay",
    description: "Complete final NDC check for each prescription before dispensing\nConfirm copays — call pts for new CC info, add to Eagle, leave AR note (ok to autocharge, call before, etc.)\nVerify all orders reach 'Submitted' status in F3 after PV2 — decline and reprocess any in prod disp",
    role: "pv2_tech",
    frequency: "daily",
    category: "operations",
  ...
  // ── DELIVERY TECH · DAILY ───────────────────────────────────────────────
  {
    ...
    description: "Open register\nLog refrigerator and freezer temperatures on Temp Log Clipboard\nLog ambient humidity on Temp Log Clipboard",
    role: "delivery_tech",
    frequency: "daily",
    category: "achc",
  ...
  },
  {
    ...
    description: "Confirm central fulfillment packages received — compare against manifest, sort will call vs. delivery\nCheck in CRC orders in CF Queue and merge packages as needed",
    role: "delivery_tech",
    frequency: "daily",
    category: "operations",
  ...
  },
  {
    ...
    description: "Contact patients for will-call pick-up — write RTS date (14 days from billed) and first contact date on packages\nSet up PSL deliveries before scheduled AM cutoff\nSet up FedEx/USPS orders by 1:30 PM\nSet up driver delivery routes — verify complete orders in MCK, note alt phone numbers, check copay/priority notes",
    role: "delivery_tech",
    frequency: "daily",
    category: "operations",
  ...
  },
  {
    ...
    description: "Confirm Cardinal invoice and MCK purchase order — verify items received, document discrepancies\nAR Report — check copay billing accuracy, bill CCOF if applicable, reach out to pts for outstanding balances\nRX Not Scanned Report — run BI portal (Pharmacy > RX Not Scanned in Eagle POS), scan and charge barcodes, collect payment or AR-bin\nWaiting in Bin Report — run Core Reports > Waiting in Bin (Days=7), send reminder calls/texts to patients\nReturn to Stock — run RTS report (Tools > Rx Tools > Return to Stock), pull unclaimed meds, print RTS labels",
    role: "delivery_tech",
    frequency: "daily",
    category: "operations",
