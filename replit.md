...
-   **Authentication**: Session-based authentication is implemented using `express-session`, supporting user roles such as Chief Pharmacy Officer (CPO), Regional Pharmacy Director (RPD), Pharmacy Director, and various Pharmacy Technician roles. All demo/staff credentials use password `AHF1`. AHF staff accounts: `claire.wood@aidshealth.org` (Tech), `pairiss.wilcox@aidshealth.org` (Tech), `anh.do@aidshealth.org` (Tech), `debbie.nguyen@aidshealth.org` (Pharmacist), `seth.collins@aidshealth.org` (Pharmacy Director), `uyen-vy.nguyen@aidshealth.org` (Pharmacist), `roshanak.mohaghegh@ahfrx.org` (Pharmacist). System accounts: `cpo@aidshealth.org` / `cpo@koheez.ai` (CPO), `regional@aidshealth.org` / `regionaldirector@koheez.ai` (RPD), `negar.shirazpour@aidshealth.org` (RPD - Western Region), `director@koheez.ai` (PD), `techs@koheez.ai` (all tech roles).
